...
use crate::db::users::{get_all_users, get_user_by_id, get_leaderboard, get_leaderboard_by_round, get_leaderboard_breakdown, get_user_data_counts};
...
async fn users_get_leaderboard_by_round(data: web::Data<AppState>, path: web::Path<String>) -> impl Responder {
    let pool: &MySqlPool = &data.pool;
    let round = path.into_inner();

    if round != "all" && !["ro64", "ro32", "ro16", "qf", "sf", "f", "gf"].contains(&round.as_str()) {
        return HttpResponse::BadRequest().body("Invalid round");
    }

    let leaderboard = if round == "all" {
        get_leaderboard(pool).await
    } else {
        get_leaderboard_by_round(pool, &round).await
    };
    match leaderboard {
        Ok(users) => HttpResponse::Ok().json(users),
...
pub fn users_controller() -> actix_web::Scope {
    web::scope("/users")
        ...
        .service(users_get_leaderboard_by_round)
