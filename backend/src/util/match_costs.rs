// Adapted from Bathbot: https://github.com/MaxOhn/Bathbot
// ISC License
...
use std::{
    collections::{HashMap, HashSet}, error::Error, mem
};

use rosu_v2::prelude::{
    GameMod, GameModIntermode, GameMods, GameModsIntermode, MatchGame, MatchTeam, Osu, OsuMatch, OsuResult, TeamType, User
};

fn get_osu_match_id(match_url: &str) -> Option<u32> {
    let parts: Vec<&str> = match_url.trim_end_matches('/').split('/').collect();
    parts.last()?.parse::<u32>().ok()
}

pub async fn matchcosts(match_url: &str, warmups: usize) -> Result<MatchResult, Box<dyn Error>> {

    let Some(match_id) = get_osu_match_id(&match_url) else {
        return Err("Invalid match URL".into());
    };

    let ez_mult = 1.0;
    
    let client_id = std::env::var("OAUTH_CLIENT_ID")?.parse::<u64>()?;
...
#[derive(Default)]
struct UsersMods {
    entries: HashMap<u32, HashSet<GameModsIntermode>>,
}
...
struct UsersPerformanceCosts {
    entries: HashMap<u32, Vec<PerformanceCost>>,
}
...
impl UsersPerformanceCosts {
    ...
    ) -> HashMap<u32, MatchCostEntry> {
        let mut match_costs = HashMap::with_capacity(self.entries.len());
